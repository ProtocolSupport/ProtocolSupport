...
public class ContainerEnchantTable extends net.minecraft.server.v1_9_R1.ContainerEnchantTable {
	...
	public void a(final IInventory iinventory) {
		if (iinventory == enchantSlots) {
			final net.minecraft.server.v1_9_R1.ItemStack itemstack = iinventory.getItem(0);
			if (itemstack != null) {
				int bookShelfs = 0;
				for (int z = -1; z <= 1; ++z) {
					for (int x = -1; x <= 1; ++x) {
				...
				for (int i = 0; i < 3; ++i) {
					if (costs[i] > 0) {
						...
						if ((list != null) && !list.isEmpty()) {
							...
							this.h[i] = net.minecraft.server.v1_9_R1.Enchantment.getId(weightedrandomenchant.enchantment);
	...
	public boolean a(final EntityHuman entityhuman, final int slot) {
		...
		net.minecraft.server.v1_9_R1.ItemStack itemstack = enchantSlots.getItem(0);
		net.minecraft.server.v1_9_R1.ItemStack lapis = enchantSlots.getItem(1);
		...
		if ((costs[slot] > 0) && (itemstack != null) && (((entityhuman.expLevel >= cost) && (entityhuman.expLevel >= costs[slot])) || entityhuman.abilities.canInstantlyBuild)) {
			...
			for (final WeightedRandomEnchant enchantment : enchantments) {
				enchants.put(org.bukkit.enchantments.Enchantment.getById(net.minecraft.server.v1_9_R1.Enchantment.getId(enchantment.enchantment)), enchantment.level);
			}
			final CraftItemStack item = CraftItemStack.asCraftMirror(itemstack);
			...
			for (final Map.Entry<Enchantment, Integer> entry : event.getEnchantsToAdd().entrySet()) {
				try {
					if (isBook) {
						...
						if (net.minecraft.server.v1_9_R1.Enchantment.c(enchantId) == null) {
							continue;
						}
						final WeightedRandomEnchant enchantment = new WeightedRandomEnchant(net.minecraft.server.v1_9_R1.Enchantment.c(enchantId), entry.getValue());
			...
			if (!entityhuman.abilities.canInstantlyBuild && supportsLapisSlot) { //ignore lapis remove for clients that don't support this slot
				final net.minecraft.server.v1_9_R1.ItemStack itemStack = lapis;
	...
	private List<WeightedRandomEnchant> getEnchantments(final net.minecraft.server.v1_9_R1.ItemStack itemstack, final int slot, final int cost) {
