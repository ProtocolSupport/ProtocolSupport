...
public class ItemStackRemapper {
	...
	public static NetworkItemStack remapToClient(ProtocolVersion version, String locale,  NetworkItemStack itemstack) {
		...
		if (version.isBefore(ProtocolVersion.MINECRAFT_1_13)) {
			int legacyCombinedId = LegacyItemIdData.getLegacyCombinedIdByModernId(itemstack.getTypeId());
			itemstack.setTypeId(LegacyItemIdData.getIdFromLegacyCombinedId(legacyCombinedId));
			itemstack.setLegacyData(LegacyItemIdData.getDataFromLegacyCombinedId(legacyCombinedId));
		}
		return itemstack;
	}
	...
	public static NetworkItemStack remapFromClient(ProtocolVersion version, String locale, NetworkItemStack itemstack) {
		if (version.isBefore(ProtocolVersion.MINECRAFT_1_13)) {
			itemstack.setTypeId(LegacyItemIdData.getModernIdByLegacyIdData(itemstack.getTypeId(), itemstack.getLegacyData()));
		}
