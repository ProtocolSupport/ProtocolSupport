...
public class EntityStatus extends MiddleEntityStatus implements
...
IClientboundMiddlePacketV20  {
	...
	protected void write() {
		io.writeClientbound(create(entity.getId(), status));
	}

	public static ClientBoundPacketData create(int entityId, int status) {
		ClientBoundPacketData entitystatusPacket = ClientBoundPacketData.create(ClientBoundPacketType.PLAY_ENTITY_STATUS);
		entitystatusPacket.writeInt(entityId);
		entitystatusPacket.writeByte(status);
