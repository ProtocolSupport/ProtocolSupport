...
public class InventorySetSlot extends MiddleInventorySetSlot {
	...
	protected void write() {
		...
		if (windowId == WINDOW_ID_PLAYER_CURSOR) {
			codec.writeClientbound(create(version, locale, windowId, stateId, slot, itemstack));
			return;
		}
		...
		try {
			...
			codec.writeClientbound(create(version, locale, windowSlot.getWindowId(), stateId, windowSlot.getSlot(), itemstack));
