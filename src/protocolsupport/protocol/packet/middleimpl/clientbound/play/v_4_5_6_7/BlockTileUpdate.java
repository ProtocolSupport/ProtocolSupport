...
public class BlockTileUpdate extends AbstractBlockTileUpdate {
	...
	public void writeToClient() {
		codec.write(create(codec, version, cache.getAttributesCache().getLocale(), tile));
	}

	public static ClientBoundPacketData create(PacketDataCodec codec, ProtocolVersion version, String locale, TileEntity tile) {
		TileEntityType type = tile.getType();
		if (type == TileEntityType.SIGN) {
			ClientBoundPacketData blocksignupdate = codec.allocClientBoundPacketData(PacketType.CLIENTBOUND_LEGACY_PLAY_UPDATE_SIGN_ID);
