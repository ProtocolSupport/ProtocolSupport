...
public abstract class AbstractLoginListener extends LoginListener {
			...
			public void run() {
				try {
					...
					if (isOnlineMode) {
						...
						networkManager.sendPacket(new PacketLoginOutEncryptionBegin("", Utils.getServer().O().getPublic(), randomBytes));
					} else {
	...
	public void a(final PacketLoginInEncryptionBegin packetlogininencryptionbegin) {
		...
		loginprocessor.execute(new Runnable() {
			...
			public void run() {
				try {
					final PrivateKey privatekey = Utils.getServer().O().getPrivate();
